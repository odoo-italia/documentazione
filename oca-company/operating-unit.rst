.. FILE AUTOMATICALLY GENERATED. DO NOT EDIT

:banner: banners/operating-unit.jpg

===============
Unità operative
===============



------
Moduli
------

|

.. toctree::
   :titlesonly:

   operating-unit/agreement_operating_unit
   operating-unit/mrp_operating_unit
   operating-unit/operating_unit
   operating-unit/fieldservice_operating_unit
   operating-unit/hr_payroll_account_operating_unit
   operating-unit/hr_contract_operating_unit
   operating-unit/sale_operating_unit
   operating-unit/stock_operating_unit
   operating-unit/purchase_request_operating_unit
   operating-unit/product_operating_unit
   operating-unit/mis_builder_budget_operating_unit
   operating-unit/mail_template_operating_unit
   operating-unit/stock_account_operating_unit
   operating-unit/sales_team_operating_unit
   operating-unit/report_qweb_operating_unit
   operating-unit/res_partner_operating_unit
   operating-unit/sale_multi_operating_unit
   operating-unit/mis_builder_operating_unit
   operating-unit/analytic_operating_unit
   operating-unit/project_operating_unit
   operating-unit/hr_operating_unit
   operating-unit/hr_expense_operating_unit
   operating-unit/purchase_operating_unit
   operating-unit/sale_stock_operating_unit
   operating-unit/crm_operating_unit
   operating-unit/account_operating_unit
